...
impl Map {
    ...
    pub fn is_tile_occupied<T: Into<Position>>(&self, position: T) -> bool {
        let position = position.into();
        self.in_bounds(position) && self.occupation[self.map_idx(position.x, position.y)] == None
    }
                            ...
                            if let Some(t) = item_mob_layer.get_tile(x as i32, y as i32) {
                                ...
                                let mut spawn_template = SpawnTemplate {
                                    ...
                                    position: Some(Position::new(
                                        x as i32,
                                        (layer.height().unwrap() - y - 1) as i32,
                                    )),
                                    index: id as usize,
                                    ..default()
                                };
...
pub fn spawn_map_tiles(map: Res<Map>, mut commands: Commands, atlas: Res<MapAsset>) {
...
pub fn spawn_map_mob_items(
    ...
    if let Some(spawn_templates) = spawn_templates {
        spawn_templates.spawn_entities(
            &mut commands,
            atlas,
            &mut map,
            current_time,
