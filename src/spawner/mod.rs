    ...
    fn spawn_entity(
        ...
        if let Some(mob_status) = mob_status {
            ...
            if let Some(hunger) = mob_status.hunger {
                entity.insert(Hunger {
                    current: hunger,
                    max: hunger,
                });
            }
            if let Some(sleep) = mob_status.sleep {
                entity.insert(SleepDesire {
                    current: sleep,
                    max: sleep,
                });
            }
            ...
            if let Some(move_id) = mob_status.move_id {
                entity.insert(GetATurn {
                    current_time: current_time.time.clone(),
                    before_time: current_time.time.clone(),
                });
                entity.insert(MoveType { move_id });
            }
            let additional_status = mob_status.additional_status.clone();
            match mob_status.mob_type {
                MobType::Plant => {
                    let additional_status = additional_status.unwrap();
                    entity.insert(Plant {
                        growth: additional_status[0],
